...
(defcustom org-table-filter-function-bindings
  '(((num (x) (string-to-number x)) . "Convert a string to a number.")
    ...
    ((field (&optional roffset coffset)
	    (let ((row (1- (+ currentline (or roffset 0))))
		  (col (1- (+ currentcol (or coffset 0)))))
	      (when (and (< -1 row numdlines) (< -1 col numcols))
		(nth col (nth (nth row table-dlines) table)))))
    ...
    ((movecell (dir &optional roffset coffset)
		 ...
		 (when (and (<= 0 line numdlines) (<= 0 col numcols)
			    (<= 0 line2 numdlines) (<= 0 col2 numcols))
		   (save-excursion (org-table-goto-line line)
				   (org-table-goto-column col)
				   (org-table--move-cell dir)
				   (let* ((f1 (nth (1- col) (nth (nth (1- line) table-dlines) table)))
