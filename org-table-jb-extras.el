...
In both these cases STEPS is set to 1."
  ...
  (let* ((table (org-table-to-lisp))
	 ...
	 (move-right (function (lambda nil (if (/= currentcol numcols)
					       (incf currentcol)
					     (setq currentcol 1)
					     (if (= currentline numdlines)
						 (setq currentline 1)
					       (incf currentline))))))
	 (move-left (function (lambda nil (if (/= currentcol 1)
					      (decf currentcol)
					    (setq currentcol numcols)
					    (if (= currentline 1)
						(setq currentline numdlines)
					      (decf currentline))))))
	 (move-up (function (lambda nil (if (/= currentline 1)
					    (decf currentline)
					  (setq currentline numdlines)
					  (if (= currentcol 1)
					      (setq currentcol numcols)
					    (decf currentcol))))))
	 (move-down (function (lambda nil (if (/= currentline numdlines)
					      (incf currentline)
					    (setq currentline 1)
    ...
    ;; TODO: this needs to be faster for large tables, use elp to profile this function.
    (while (< matchcount (abs steps))
      (funcall movefn)
      (setq cellcount 1)
      (while (and (or (/= currentcol startcol)
		      (/= currentline startline))
