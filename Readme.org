...
If you want more fine-grained control you can use *org-table-flatten-columns* to select exactly which cells should be combined, and how they should be combined (by appending, prepending, summing values, etc). When used in combination with *org-table-jump-next* (see below) and keyboard macros this can be a very powerful tool.
