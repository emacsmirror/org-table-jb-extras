...
Example: src_emacs-lisp{(0 :empty 1 :nonempty :hline-below 2 :empty -> :nonempty :hline-above)}
...
Jump sequences allow different jump conditions to be used in sequence. They can be nested, and nested sequences move one step forward for each complete iteration of the parent sequence. For example:
 ...
 - src_emacs-lisp{(A -> B -> C)} results in A,B,C,A,B,C,etc.
 - src_emacs-lisp{(A -> (B -> C) -> D)} results in A,B,D,A,C,D,A,B,D,etc.
 - src_emacs-lisp{(A -> (B -> (C -> D)) -> E)} results in A,B,E,A,C,E,A,B,E,A,D,E,A,B,E,etc.
