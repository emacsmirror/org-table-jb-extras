...
This line should come directly after the table and any "#+TBLFM:" lines.
...
Jump conditions are defined using a special syntax explained in the docstring for *org-table-jump-condition* which is displayed when you are prompted to enter a condition. In a jump condition definition you can make use of functions defined in *org-table-filter-function-bindings*, e.g. *hline-p* (check for horizontal lines), *matchfield* (match a field against a regexp), *countcells* (count neighbouring cells matching regexps), and also functions to alter the table, e.g. *setfield* (change a field), *replace-in-field* (replace substring of field) *convertdate* (change the format of a date string in field), *flatten* (combine cells), etc.
