...
Jump conditions may pass information from one jump to the next using *org-table-jump-state* which allows you to perform more complex tasks.
This is done using *getvar,setvar* & *checkvar* functions.
...
containing "bar". This is acheived by using a state variable x, which is initialized to 1 at the start.
#+BEGIN_SRC emacs-lisp
  ((null (getvar 'x)) (setvar 'x 1) "foo" | (checkvar 'x 1) "foo" (:empty-below (setvar 'x 2) | t) | (checkvar 'x 2) "bar")
#+END_SRC
