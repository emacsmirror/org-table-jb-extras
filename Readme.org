...
These presets can be selected, altered ("edit preset"), or you can define you own ("enter manually") using an arbitrary lisp expression, by either use a C-u prefix arg with *org-table-jump-next/org-table-jump-prev* or calling *org-table-set-jump-condition*.
...
When defining your own jump condition you can make use of functions defined in *org-table-filter-function-bindings*. These functions may pass information from one jump to the next using *org-table-jump-state* which allows you to perform more complex tasks such as solving mazes. The file [[https://github.com/vapniks/org-table-jb-extras/blob/main/org-table-solve-mazelog.org][org-table-solve-mazelog.org]] contains examples of this.
