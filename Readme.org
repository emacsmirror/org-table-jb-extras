...
These presets can be selected, altered, or you can define you own by either pressing *C-u* followed by  *M-x org-table-jump-next/org-table-jump-prev* or pressing *M-x org-table-set-jump-condition*, or can customize *org-table-jump-condition-presets* if you want to use the condition again in the future.
...
Jump conditions are defined using a special syntax explained in the docstring for *org-table-jump-condition* which is displayed when you are prompted to enter a condition. In a jump condition definition you can make use of functions defined in *org-table-filter-function-bindings* to help define the condition, e.g. *hline-p* (check for horizontal lines), *matchfield* (match a field against a regexp), *countcells* (count neighbouring cells matching regexps), and also functions to alter the table, e.g. *setfield* (change a field), *replace-in-field* (replace substring of field) *convertdate* (change the format of a date string in field), *flatten* (combine cells), etc.

For example the following form can be used to flatten cells to the right of empty ones by joining them with the first cell above that isnt to the right of an empty one:
#+BEGIN_SRC  emacs-lisp :results none
(& "\\S-" ("\\S-" 0 -1) ("^\\s-*$" 1 -1) (flatten (1+ (sumcounts 'down 1 -1 "^\\s-*$")) 1))
#+END_SRC
To use it, put point in an org-table, do *C-u M-x org-table-jump-next*, choose "Enter manually" from the menu, and enter the form above. It will transform this table:
| foo | bar  |
|     | choo |
|     | zoo  |
| aaa | bbb  |
